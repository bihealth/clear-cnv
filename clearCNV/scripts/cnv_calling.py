...
#D0 = util.load_dataframe(intsv_path,util.load_annotated(annotated_path))
...
Matchscores = pd.read_csv(matchscores_path,
                          sep='\t',
                          low_memory=False,
                          header=0,
                          index_col=0)
...
suggested_MINIMUM_SAMPLE_SCORE = np.mean(Matchscores.sum(axis=0) / (len(Matchscores.columns)-1))
...
n, bins, patches = plt.hist(sample_group_sizes,int(len(sample_group_sizes)/2),label=str(len(failed_samples))+" failed samples")
plt.vlines(MINIMUM_SAMPLE_GROUP,0,max(n)*0.75, label="cutoff at %.d"%MINIMUM_SAMPLE_GROUP,color='darkred')
...
print("%.d samples failed to have a sufficient group size. They are ignored in cnv calling."%len(failed_samples))
...
util.print_clustermap(Matchscores,
...
def collect_result(result):
    sample_id = result[0]
    index = result[1]
    global sample_scores
    global ratio_scores
    global z_scores_scaled
...
for i in range(len(Matchscores_bools.columns)):
    pool.apply_async(util.calling_cnv, args=(i, DA[0], Matchscores_bools, DA[1], EXPECTED_CNV_RATE,SENSITIVITY), callback=collect_result)
...
if sum(DX[1]) > 0 and np.max(DX[0].mean(axis=0)) / np.min(DX[0].mean(axis=0)) > 1.5:
    ...
    pool = mp.Pool(CORES)
    for i in range(len(Matchscores_bools.columns)):
        pool.apply_async(util.calling_cnv, args=(i, DX[0], Matchscores_bools, DX[1], EXPECTED_CNV_RATE,SENSITIVITY), callback=collect_result)
    pool.close()
    pool.join()
...
if sum(DY[1]) > 0 and np.max(DY[0].mean(axis=0)) / np.min(DY[0].mean(axis=0)) > 1.5:
    ...
    pool = mp.Pool(CORES)
    for i in range(len(Matchscores_bools.columns)):
        pool.apply_async(util.calling_cnv, args=(i, DY[0], My, DY[1], EXPECTED_CNV_RATE,SENSITIVITY), callback=collect_result)
    pool.close()
    pool.join()
...
#np.max(z_scores_scaled),np.min(z_scores_scaled)


# In[49]:


cnv_carrying_indexes = np.array(range(z_scores_scaled.shape[0]))[[sum((s != 1)) > 0 for s in HMM]]
...
pool = mp.Pool(CORES)
CNVs = list(zip(cnv_carrying_indexes,
        [pool.apply(util.merge_score_cnvs,
...
#util.npcolor(z_scores_scaled)


# In[65]:


#plt.hist(z_scores_scaled[1],20)
...
pd.DataFrame([[SAMPLES[sample_cnv[0]],*cnv.to_list(),sample_scores[CNVs[i][0]]] for
              i,sample_cnv in enumerate(CNVs) for
              cnv in sample_cnv[1]],
            columns=["sample","chr","start",
                     "end","gene","aberration",
                     "size","score","sample_score"]
            ).sort_values(by="score",ascending=False).to_csv(calls_path,sep='\t',index = False)


# In[66]:


#pd.DataFrame(z_scores_scaled,columns=INDEX,index=SAMPLES).T.loc["15_48936798_48936966_FBN1","17_1123-N1-DNA1-Panel_seq1"]


# In[93]:


pd.DataFrame(z_scores_scaled,columns=INDEX,index=SAMPLES).T.to_csv(analysis_dir+'z_scores.tsv',sep='\t')
pd.DataFrame(ratio_scores,columns=INDEX,index=SAMPLES).T.to_csv(analysis_dir+'ratio_scores.tsv',sep='\t')
