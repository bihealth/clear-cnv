...
def load_dataframe(path,sep='\t',annotated=None):
    ...
    if not (D0.columns[0] in ['chr','Chr','chromosome','Chromosome'] and 
        D0.columns[1] in ['start','Start'] and 
        D0.columns[2] in ['end','End'] and 
        D0.columns[3] in ['gene','Gene']):
            raise Exception("File needs to be formatted like: chr\tstart\tend\tgene\tsample_0\tsample_1\tsample_n")
...
def trim(D,axis=0,alpha=0.1):
    if axis:
        return D[trimmed_std_np(D,axis=axis) < np.quantile(trimmed_std_np(D,axis=axis),1-alpha),:]
    else:
        return D[:,trimmed_std_np(D,axis=axis) < np.quantile(trimmed_std_np(D,axis=axis),1-alpha)]
...
def hmm_predict(D):
    ...
    model.transmat_ = np.array([[0.999, 0.001, 0.0],
...
def hmm_scores(X, means=[[-5], [0.0], [5]], transitionprobs=[[0.999, 0.001, 0.0],
                                                            [0.0001, 0.9998, 0.0001],
                                                            [0.0, 0.001, 0.999]]):
    model = hmm.GaussianHMM(n_components=3, covariance_type="full")
    model.startprob_ = np.array([0.0001, 0.9998, 0.0001])
    ...
    model.covars_ = np.tile(np.identity(1), (3, 1, 1))
    return model.predict([[s] for s in X])
...
def merge_score_cnvs(hmm_preds,z_scores,r_scores,index):
    ...
    for i in range(1,len(hmm_preds)):
        if hmm_preds[i] != hmm_preds[i-1]:
            ...
            # cnv -> w or cnv
            if hmm_preds[i-1] != 1:
                CNVs.append(ca.CNV(*index[start].split('_')[:2],
...
def calling_cnv(index_sample, D, MS, index, EXPECTED_CNV_RATE, SENSITIVITY = 0.6):
    # de-bias samples
    # check if it is probable that more than one gene is present,
    # then re-normalize within each sample.
    if D[0].shape[0] > 50:
        D = normalize(D,axis=0)
    D = normalize(D,axis=1)+1-np.median(normalize(D,axis=1),axis=0)
    ...
    zscores      = (ratio_values-1) / (trimmed_std_np(D3, axis=1,alpha=EXPECTED_CNV_RATE)**(1-SENSITIVITY+1))

    sample_score = 1/(trimmed_std_np(zscores[:,None], axis=0,alpha=EXPECTED_CNV_RATE)[0]**(1-SENSITIVITY+1))
