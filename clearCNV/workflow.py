...
def workflow(args):
    configpath = os.path.dirname(os.path.abspath(__file__)) / pathlib.Path("workflow") / pathlib.Path("config.yml")
    create_config(configpath, args)
    subprocess.check_call(["snakemake",
                        "--snakefile",
                        str(os.path.dirname(os.path.abspath(__file__)) / pathlib.Path("workflow") / pathlib.Path("Snakefile")),
                        "--configfile",
                        str(configpath)])
