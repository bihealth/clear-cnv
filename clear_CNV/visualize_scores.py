...
def visualize(args):
    ...
    def splits(index,htmp_size):
        ...
        if len(chrs) <= 1 or len(index) <= htmp_size:
            ...
            return [*splits(index[:chrstarts[:split_pos][-1]],htmp_size),*splits(index[chrstarts[:split_pos][-1]:],htmp_size)]
    def get_plots(df,annotated):
        n_clusters=math.ceil(len(df.columns)/40)
        kmeans = KMeans(n_clusters=n_clusters, random_state=0).fit(df[df.std(axis=1) < np.nanquantile(df.std(axis=1),0.95)].T)
    ...
    for i,split in enumerate(splits(RS.index, htmp_size)):
        plotly.offline.plot(get_plots(RS.loc[split,:],annotated), filename=analysis_dir+f"ratio_scores_extended_{i}.html")
