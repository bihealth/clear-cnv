...
class Target:
    ...
    def __eq__(self, other):
        ...
        return (self.chr == other.chr and                 other.start > al and other.start < ar and                 other.end > bl and other.end < br)
    def __lt__(self, other):
        if self.chr == other.chr:
            return self.start+(self.end-self.start)/2 < other.start+(other.end-other.start)/2
...
class CNV:
    ...
    def __str__(self):
        if self.size:
            return '\t'.join(map(str,[self.chr,self.start,self.end,self.gene,self.abb,self.score,self.size]))
        return '\t'.join(map(str,
[self.chr,self.start,self.end,self.gene,self.abb,self.score]))
    ...
    def __eq__(self, other):
        ...
        al,ar = self.start - alpha, self.start + alpha
        bl,br = self.end - alpha, self.end + alpha
        return (self.chr == other.chr and                 other.start > al and other.start < ar and                 other.end > bl and other.end < br and                 self.abb == other.abb)
    def __ne__(self,other):
        return not self == other
    def to_list(self):
        if self.size:
               return list(map(str,[self.chr,self.start,self.end,self.gene,self.abb,self.score,self.size]))
